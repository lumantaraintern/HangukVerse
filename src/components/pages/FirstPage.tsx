...
export default function FirstPage({ onNext }: FirstPageProps) {
  ...
  // Ref to ensure toast only fires once
  ...
  useEffect(() => {
    const checkSession = async () => {
      ...
      if (user?.email && !toastShown.current) {
        setUserEmail(user.email)
        toast.success(`Already signed in as ${user.email}`)
        toastShown.current = true // prevent future toasts
      }
    }
    checkSession()
  }, [supabase])
  ...
  return (
    <section className="relative w-full min-h-screen overflow-y-auto overflow-x-hidden">
      <div className="relative w-full aspect-[1440/1024] bg-[#0b0813]">
        {/* Background */}
        <Image src="/Hangukverse_bg_Nav1.jpg" alt="" fill className="absolute inset-0 object-contain object-top z-30" />
        <Image src="/exterior.png" alt="" fill className="absolute inset-0 object-contain object-center z-20" />
        ...
        {/* Interior */}
        <Image src="/interior.png" alt="" fill className="absolute inset-0 object-contain object-center z-10" />
        <Image src="/liftdoorl.png" alt="" fill className="absolute inset-0 object-contain object-center z-5" />
        <Image src="/liftdoorr.png" alt="" fill className="absolute inset-0 object-contain object-center z-5" />
        ...
        {/* Click Me Button */}
        {!showAuth && (
          <div className="absolute z-50 left-1/2 top-[65%] -translate-x-1/2 -translate-y-1/2">
            <Button
